...
pool = ParserPool(outdir='test/output', nSimultaneous=3, nReadersPerFile=2, nParsersPerFile=4)
...
with open("test/list.txt") as f:
    for line in f:
        url = line.strip()
        fn = url.split("/")[-1]
        print(f"\033[0HDownloading {fn}...", end='\r', flush=True)
        fn = os.path.join('test', fn)
        if not os.path.exists(fn):
            subprocess.call(["curl", "-o", fn, url], stdout=devnull, stderr=devnull) 
        print("\033[0HFinished downloading".rjust(100), end='\r', flush=True)
